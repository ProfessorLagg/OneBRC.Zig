...
pub fn brcIntParseAsm(str: []const u8) i16 {
    return asm volatile (
        \\xor %rdx, %rdx
        // now ax and rdx contain 0

        \\cmpb $45, (%rsi)
        \\sete %dl
        // Now dl / rdx contains 1 if negative 0 if positive

        \\mov $-2, %bx
        \\imul %dx, %bx
        \\add $1, %bx
        // Now bx contains -1 if negative or +1 if positive

        \\add %rcx, %rsi
        \\dec %rsi
        // Now rsi contains address of last char in the array
        ...
        \\movb (%rsi), %al
        \\sub $48, %al
        // Now %ax contains the absolute value of the 1s position

        \\sub $2, %rsi
        \\xor %r8w, %r8w
        \\movb (%rsi), %r8b
        \\sub $48, %r8b
        // now r8w contains the absolute value of the 10s position

        \\xor %r9w, %r9w
        \\cmp $4, %rcx
        \\jne .skip100s # skip doing the 100s position if length != 4
        \\dec %rsi
        \\movb (%rsi), %r9b
        \\sub $48, %r9b
        // now r9w contains the absolute value of the 100s position
